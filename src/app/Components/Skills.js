            ...
            <div>
                ...
                <div className='flex justify-center mt-12 mb-20'>
                    <div className="grid grid-cols-5 grid-rows-3 gap-3 sm:gap-5 justify-center max-w-full sm:max-w-4xl">
                        {skills.map((skill) => (
                            <div key={skill.name} className={`relative group/skill transition-all w-14 h-14 sm:w-16 sm:h-16 ${activeCategory && activeCategory !== skill.category  ? "blur-sm opacity-50" : "opacity-100"}`}>
                                <img src={skill.img} alt={skill.name} className="rounded-md shadow-md w-12 h-12 sm:w-14 sm:h-14 object-contain " />
