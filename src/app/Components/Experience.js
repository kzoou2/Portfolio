export default function Experience(){
    ...
    return(
        <section id='experience' className="w-full scroll-mt-[10vh]">
            <div>
                <h2 className="section-index">경력 사항</h2>
                <p className="section-label">안녕하세요 테스트입니다</p>
            </div>
            
            <div className="md:max-w-[768px] mx-auto mb-20">
                {Object.entries(grouped).map(([type, items]) => (
                    <div key={type} className="mb-12">
                        ...
                        <div className="flex flex-col gap-8 md:gap-10">
                            {items.map((item, idx) => (
                                <div key={idx} className="grid sm:grid-cols-3 sm:gap-x-10 sm:items-start">
                                    ...
                                    <div className="pl-6 sm:pl-0 sm:col-span-2 flex flex-col gap-3">
                                        <div className="flex flex-col gap-1">
                                            ...
                                            {item.readme && (
                                                <ul className="list-disc pl-5 text-xs md:text-sm text-foreground/60">
                                                {item.readme.map((r, i) => (
                                                    <li key={i}>{r}</li>
                                                ))}
                                                </ul>
