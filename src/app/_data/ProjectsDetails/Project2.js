    ...
    parts:[
        {
            ...
            items:[
                ...
                "프로젝트의 시각적 통일성과 사용성 개선을 위한 디자인 방향 제시",
        ...
        },
        {
            ...
            '웹 표준 및 접근성 고려한 컴포넌트 구조 설계로 전반적인 사용성 개선',
            ],
        },
        {
            title:'실시간 채팅 기능 구현',
            items:[
                "STOMP 기반 WebSocket을 활용해 1:1 실시간 메시지 송수신 기능 구현",
                "채팅방 생성 및 기존 채팅방 재연결 로직을 구현하고 채팅방 전환시 구독 관리로 안정적인 연결 유지",
        ...
        },
        {
            ...
            items:[
                "React와 Axios를 활용해 로그인·회원가입 전반의 인증 흐름을 설계하고 구현",
                "이메일·비밀번호 유효성 검사 및 에러 메시지 분기 처리로 사용자 입력 오류 대응",
                "로그인 성공 시 Access Token / Refresh Token 저장 및 인증 상태 로직 구현",
                "토큰 만료를 고려한 재발급 로직을 구현하여 자동 로그인 환경 구성"
            ]
        },
        {
            title:'로그인 상태에 따른 공통 화면 구성',
            items:[
                "로그인 여부에 따라 Navbar 및 메인 홈 UI를 동적으로 구성",
                "로그인 시 프로필과 닉네임 표시, 비로그인 시 페이지 접근 제어 처리",
                "localStorage를 활용해 로그인 상태를 관리하고, 공통 컴포넌트 구조를 통해 상태 변화가 전역 UI에 반영되도록 설계"
            ],
        },
        {
            title:'ReactPlayer기반 음악 플레이어 기능 구현',
            items:[
                "Recoil 전역 상태 관리로 재생 상태·볼륨·진행률 등 플레이어 상태 동기화",
